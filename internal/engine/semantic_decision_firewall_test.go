...
func TestSemanticDecisionFirewallPrioritizesTouchingBeforeTruncation(t *testing.T) {
	...
	for i := range scores {
		...
		}}}, "firewall-priority", "test"); err != nil {
			t.Fatal(err)
		}
	}
...
func TestSemanticDecisionFirewallKeepsExactRiskWithoutEmbeddingModel(t *testing.T) {
	e, _ := initEngine(t, map[string]string{"worker.go": "package worker\n\nfunc Run() {}\n"})
	const marker = "deterministic-firewall-marker"
	if _, err := e.Remember(RememberArgs{Node: "worker.go#Run", Entries: []RememberEntry{{
		Kind: model.KindPitfall, Text: marker + " 必须复用幂等键",
	}}}, "firewall-lexical", "test"); err != nil {
		t.Fatal(err)
	}
	out, err := e.TaskContext(context.Background(), TaskArgs{Action: "start", WIP: model.WIP{
		Task: marker, Touching: []string{"worker.go#Run"},
	}}, "firewall-lexical", "test")
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{"语义决策防火墙", "keyword-risk", "worker.go#Run", "仅告警，不阻断"} {
		if !strings.Contains(out, want) {
			t.Fatalf("model-free firewall missing %q:\n%s", want, out)
		}
	}
}

func TestSemanticDecisionFirewallChecksEverySplitHeirWithoutQueryMatch(t *testing.T) {
	...
	if got := strings.Count(out, "[风险·touching精确]"); got != semanticFirewallMaxExactWarnings {
		t.Fatalf("expanded exact warnings=%d, want %d:\n%s", got, semanticFirewallMaxExactWarnings, out)
	}
...
func TestSemanticDecisionFirewallChecksRejectedAndFlowWithoutEmbedding(t *testing.T) {
	e, _ := initEngine(t, map[string]string{"worker.go": "package worker\n\nfunc Run() {}\n"})
	...
	}, "firewall-no-model", "test"); err != nil {
		t.Fatal(err)
	}
...
func TestSemanticDecisionFirewallWarnsWithoutBlockingTaskStart(t *testing.T) {
	...
	if got := provider.requests.Load(); got != requestsBefore+1 {
		t.Fatalf("task firewall provider requests=%d, want %d", got, requestsBefore+1)
	}
	got, err := e.Task(TaskArgs{Action: "get"}, "firewall-session", "test")
	...
	}}, "firewall-provider-failure", "test")
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{"语义决策防火墙状态提示", "本次告警可能不完整", "任务不会因此阻断"} {
		if !strings.Contains(out, want) {
	...
	got, err = e.Task(TaskArgs{Action: "get"}, "firewall-provider-failure", "test")
