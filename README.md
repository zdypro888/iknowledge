...
# 3. Print the integration snippets; paste the ones you need (iknowledge only prints — it never writes your files)
...
The core snippets printed by `setup` go into these files (the Git hook is optional and is never installed automatically):
...
| `.git/hooks/pre-commit` | `iknowledge precheck` snippet | Surfaces rejected alternatives, stale/disputed knowledge, landmines, and source files not covered by newly-added journal records; warning-only by default, `--strict` is opt-in |
...
iknowledge brief --repo . --budget 1200   # framed one-screen Markdown: WIP, risks, recent decisions, maintenance debt
iknowledge precheck --repo .          # correlate staged source changes (including deletes) with new journal nodes
...
- **Is it Go-only?** Symbol-level support (function granularity, format-immune hashing, rename migration, call graph): Go is complete, **Python** is supported (parsed by your local python3 using its own ast module — zero extra installs), and **JavaScript/TypeScript** (.ts/.tsx/.js/.jsx/.mjs/.cjs), **Rust** (.rs), and **Java** (.java) are supported too — all via built-in lightweight Go lexers, zero runtime dependencies, approximate symbol extraction following the "prefer missing over wrong" principle. **Any other language**: add `extensions: [".proto", ".sql"]` to `.knowledge/config.yaml` and those files join at file granularity — ledger, experience, hook injection and rot detection (content hash) all work; you just don't get symbol drill-down or the call graph. The core engine is language-agnostic; adding a symbol-level language = writing one parser plugin.
...
- **Security model?** Listens on `127.0.0.1` only by default, no auth (local trust model); Origin validation blocks browser DNS-rebinding. A non-loopback bind is refused unless auth is enabled or `--allow-insecure-bind` explicitly accepts the risk. On shared multi-user machines use `serve --auth`: a token is generated at `.knowledge/local/token` (0600), all endpoints require `Authorization: Bearer`, and the stdio bridge automatically uses it for health checks and startup. A missing token means unauthenticated local mode; an existing malformed or over-permissive token fails closed (`serve --auth` repairs permissions). Before either stdio or the hook sends credentials, it probes without a secret and requires a fresh 256-bit nonce/HMAC-SHA256 proof of token possession; an unauthenticated listener, a forged generic Bearer challenge, or a replayed static fingerprint therefore receives no token. Restart any pre-upgrade background server once so it supports the proof. Plain loopback TCP still cannot stop an active same-host relay that forwards the live challenge and later traffic—use OS/user isolation for that stronger threat. Semantic writes and bundle imports redact high-confidence secrets before persistence and report what was removed. HTTP-direct snippets can contain the token—don't commit them. The tool remains read-only on your sources.
...
Phase 1 fully delivered and continuously hardened: now 16 MCP tools + the `/mcp/main` and `/mcp/scout` endpoints + `GET /inject` and the read-only legs (`/recall` `/map` `/status`) + the `iknowledge hook/setup/maintain/doctor/brief/precheck` suite, fixed through multiple adversarial reviews and a third-party full-repo audit. The latest hardening adds authenticated stdio auto-start without downgrade, default secret redaction on persistence/import, node-correlated staged-change prechecks (including deletions), and budgeted session briefings with an untrusted-data frame. MCP invalid requests and oversized bodies fail explicitly, and CLI/MCP release versions now share injected build metadata. On 2026-07-04 the originally-deferred phase 2/3/4 items landed: full-repo call graph & structural search expansion, hotspot digestion list, dispute registration, review reminders for non-code knowledge, `--auth`, multi-repo single daemon, Windows support (CI green on all three OSes), and the PTY self-dispatch scout fallback. **Both clients field-tested** (Claude Code + Codex, including instructions semantics). **M1.4 A/B acceptance passed**: 10 fixed code-location tasks, knowledge-base-connected (19 % seeded coverage) vs bare grep, same model — median tokens down 41 % (59 % ≤ the 60 % threshold), cheaper on 8/10 tasks, faster wall-clock; protocol, harness (`cmd/kbeval`) and both rounds of raw data live in [eval/m14/](eval/m14/).
