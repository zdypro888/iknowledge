...
Then, inside any project, tell **Claude Code or Codex**: **"initialize the knowledge base for this project"**. The AI builds the skeleton, writes all integration config for you (the Claude Code trio + Codex's project-scoped `.codex/config.toml`/`AGENTS.md`) and verifies connectivity (both clients field-tested). After restarting the session, the `kb_*` tools and hook injection are live; the server is auto-started on demand by the stdio bridge, so even a machine reboot needs no attention.
...
| `<repo>/.codex/config.toml` + repo `AGENTS.md` | Codex MCP + discipline | Project-scoped Codex integration (optional); use `~/.codex/config.toml` only when deliberately enabling this repository's MCP for every Codex project |
...
iknowledge doctor --repo . --deploy   # also audits Git truth, WIP age, process count/RSS/age and daemon build
...
"Ships with the code" is a checked invariant, not an assumption. Durable truth (`project.yaml`, `config.yaml`, `tree/`, `journal/`, `flows/`, and `topics/`) must actually enter the Git index; only `local/`, `wip/`, and temporary files stay ignored. `doctor` reports `tracked`, `partial`, `untracked`, `ignored`, or `no-git`, including an outer `.gitignore` that discards the entire `.knowledge/` directory. A non-Git umbrella workspace can still use iknowledge locally, but it cannot provide branch-aware/team durability unless that knowledge root is placed in its own Git repository.
...
You never manage the server: the stdio bridge auto-starts the background serve on demand. Internal clients always verify the current loopback listener with a mutual-HMAC challenge and send only a scope-bound short-lived session; the long-term local identity is never sent to an unknown port. Each daemon captures its executable SHA-256, revision and start time. A bridge from a newer build authenticates the old daemon, requests a graceful drain, waits for the listener to close, and starts the current build; a pre-upgrade daemon without that runtime endpoint fails closed and must be stopped once manually. `doctor --deploy` exposes the active bridge/daemon count, RSS, age and build identity. If Bearer auth was enabled before reboot, its user-private token also preserves that mode and the bridge restarts `serve --auth`. And even if everything is down, the AI just works normally and the hook stays silently inert.
...
| Write | `kb_record_change` | The change ledger: what / why / what was rejected; node syntax is normalized conservatively, source-confirmed new files/symbols are anchored atomically, and ambiguous misses return bounded canonical candidates rather than silently choosing another node |
...
| State | `kb_task` | Session-isolated work-in-progress ledger: start/update/complete/abandon, with guarded owner+reason closure for verified stale (7d+) sessions and end-of-task reminders |
...
| Maint | `kb_status` | Library health; routine semantic state stays silent, with `semantic_action` emitted only when sync is actionable |
| Maint | `kb_semantic` | Explicit offline diagnostics, or one policy-authorized safe incremental sync; a repeat in the same session is a successful zero-provider no-op, never a retry-triggering error |
...
- **Is an existing `.knowledge/` directory automatically backed up?** No. The branch/team guarantee exists only when its durable truth is actually tracked by Git. Run `iknowledge doctor --repo .`; it detects partial/untracked truth, an outer ignore rule that swallows `.knowledge/`, and knowledge roots outside any Git worktree. Local runtime state remains intentionally untracked.
- **Does semantic/vector search upload my repository or require another MCP server?** An optional **preview is implemented and disabled by default**. Loopback Ollama and remote HTTPS OpenAI-compatible endpoints are direct HTTP providers inside iknowledge, not another MCP service. The derived Flat index contains only redacted `current`, `risk`, and `history` knowledge cards—never source-code chunks. Only `current` joins lexical RRF; risk/history remain a historical-decision advisory that never blocks or adjudicates. Provider, index, or resource failures fall back to the normal lexical/structural path.

  Configuration is explicit, repository-scoped user-private state outside Git. `manual` is the default rebuild policy; `ai-local` and `ai-remote` only authorize one status-directed MCP sync attempt per session. Concurrent or later sync calls in that same session deterministically succeed as `skipped/already-attempted`, contact no provider, and cannot encourage an agent retry loop; after the first claim, that session's `kb_status` also suppresses `semantic_action`. With a fully validated old generation in the same vector space, the winning sync copies vectors whose complete record identity is unchanged and embeds only added/changed cards; deletions require no document embedding. The 3,000-card interactive limit applies to this delta, so a large repository can still perform small routine syncs. If no old generation is safe to reuse, MCP treats all current cards as pending and can still complete a full rebuild in that same attempt when they fit the limit. A CLI full rebuild is required only when the safely assessed pending work exceeds 3,000, or when integrity/canary checks make the MCP attempt unsafe to continue (for example, drift in a large generation); vector spaces are never mixed. Routine `kb_status` output omits model, record-count, and provider-probe diagnostics; an enabled state that truly needs human intervention emits only a compact `semantic_attention`. Request `kb_semantic status` explicitly when diagnosing semantic search.

  Remote keys come only from `IKNOWLEDGE_EMBEDDING_API_KEY` and must be audience-bound with `IKNOWLEDGE_EMBEDDING_API_ORIGIN`. iknowledge never installs Ollama, downloads a model, or silently changes one. The shortest local setup is:
  ...
  To authorize the AI to perform a needed local sync, choose `--rebuild-policy ai-local`; remote synchronization requires the separate explicit `ai-remote` policy and a non-loopback HTTPS endpoint. The complete implementation, safety, resource, and backend-promotion contract is [knowledge-impl.md §8.1](knowledge-impl.md#81-可选语义检索-preview已实现默认禁用). The deterministic fixture format is documented in [eval/semantic/README.md](eval/semantic/README.md); its four hand-authored cases guard the algorithm, not real-model quality, so the preview remains opt-in.
...
- [`knowledge-impl.md`](knowledge-impl.md) — the current engineering implementation contract (packages, data model, storage, MCP APIs, semantic preview, milestones) *(Chinese)*
- [`eval/semantic/README.md`](eval/semantic/README.md) — semantic retrieval regression fixture and model-quality promotion protocol
