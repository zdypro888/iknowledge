...
Then, inside any project, tell **Claude Code or Codex**: **"initialize the knowledge base for this project"**. The AI builds the skeleton, writes all integration config for you (the Claude Code trio + Codex's `config.toml`/`AGENTS.md`), starts the server, and verifies connectivity (both clients field-tested). After restarting the session, the `kb_*` tools and hook injection are live; after a machine reboot, one sentence — "start the knowledge base server" — brings it back.
