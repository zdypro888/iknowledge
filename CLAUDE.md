...
AI 代码知识库(MCP 服务)。**两份设计正本共同构成现行规范**,实现与其冲突时要么修文档(留痕)要么修实现,不许静默偏离:

- [knowledge.md](knowledge.md) — 概念设计全案(五维模型、自愈机制、两条铁律、五篇推演)
- [knowledge-impl.md](knowledge-impl.md) — 工程实现正本(数据模型、存储、MCP API、semantic/vector preview、里程碑)——**实现照这份**,已含推演五及后续定案
...
**轮 39(2026-07-20,文档职责收口)**:semantic/vector 已从设想进入稳定 preview 后,删除重复的专项文档,把概念边界归 `knowledge.md` §10.2、完整实现契约归 `knowledge-impl.md` §8.1、用户接入归双语 README、离线评测格式与晋级协议归 `eval/semantic/README.md`。现行规范恢复为两份正本,历史留痕保留但不再维护第三份重复真相源。

**轮 40(2026-07-20,大仓 semantic 增量同步与静默运行态)**:针对 casino 总卡数超过 MCP 3000 上限及 agent 反复复述 provider 状态的问题,① `kb_status` 改为静默消费,正常态不再输出 semantic 诊断,仅在既有 ai policy 下追加明确 `semantic_action`;人工介入态只给精简 `semantic_attention`,详细离线诊断归 `kb_semantic status`,以 `provider_probe: deferred` 表达未探测而非故障。② MCP sync 在完整 payload、settings/embedder/dimensions、双 canary 未检测到常见漂移后,按完整 record identity 复用旧向量,只 embedding 新增/变化卡,删除零文档 embedding,3000 上限改按 delta；无安全旧代时全部当前卡视为 pending，小仓可由同次 MCP 完整重建，只有 pending 超限或完整性/canary 不允许安全继续时才转 CLI，绝不混代。首次评估大仓 partial 会用单行缓冲流式完整校验 payload 并精确计算 pending；精确 pending/delta/canary block 按 immutable index identity+source+settings 有界缓存，已确认 payload corruption 则另按 immutable index identity 缓存，避免跨 MCP 会话重复失败。③外层 metadata 与内层 codec shape 在矩阵分配前绑定,daemon 按新旧实际矩阵字节记账,transient 的 total 与可晋升 resident 上限分离,发布报告增加 reused/embedded,并补大仓增删改、超限、漂移、双仓资源、真实 recall 与静默 UX 回归。

**轮 41(2026-08-20,本机真实使用审计后的可靠性/运维闭环)**:针对 casino/apple 的 1.6 万次工具调用、长期 daemon/多 bridge、Git 正本未跟踪、`NODE_NOT_FOUND` 记账失败与 semantic 重试噪音,落地五组修复。① `doctor` 以实际 Git top-level/check-ignore/ls-files 报 tracked/partial/untracked/ignored/no-git,并统计 WIP 24h/7d 与部署进程数/RSS/年龄/daemon build；② Codex setup 缺省改项目级 `<repo>/.codex/config.toml`+cwd,全局段仅显式共享；③ Sync 保留未变 index generation,以知识承载源码的真实字节 manifest 决定是否 parser/reconcile,journal/flow/WIP 独立变化不再误触全源码对账；④ record_change 对节点做保守规范化、严格唯一 lineage 解析、当前源码确认的新文件/符号原子落锚、稳定有界候选与重复合并，歧义/typo/排除项绝不猜锚；⑤ semantic 同 session 重复 sync 成功返回 zero-provider no-op 且 status 抑制后续 action；daemon 捕获 executable SHA256/revision,stdio 经认证 runtime endpoint 请求旧构建优雅排空并自动拉起当前构建，缺 endpoint 的历史 daemon 首次换代 fail closed。
