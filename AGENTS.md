...
- **测试 MCP:协议级手段优先**(curl / httptest,便宜无额度消耗);真实客户端联测**禁用 `Codex -p`**(独立限流池,撞账号限额),用 **PTY 驱动交互式 Codex**(aibridge `internal/agent` 模式,用户已授权)
