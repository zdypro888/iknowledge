...
- **只支持 Go 吗?** 符号级(函数粒度、格式化免疫、改名自动迁移、调用图):Go 完整支持,**Python** 已支持(用本机 python3 自己的 ast 解析,零额外安装),**JavaScript/TypeScript** 也已支持(.ts/.tsx/.js/.jsx/.mjs/.cjs——内置纯 Go 轻量词法分析,零运行时依赖,近似符号提取遵循"宁缺不要错"原则)。**其他任何语言**:在 `.knowledge/config.yaml` 加 `extensions: [".proto", ".sql"]`,即以文件粒度入库——账本、经验、hook 注入、腐烂检测(内容哈希)全可用,只是没有符号下钻和调用图。核心引擎语言无关,加一门符号级语言=写一个解析器插件。
