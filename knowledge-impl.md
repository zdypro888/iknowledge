...
iknowledge setup  --repo /path/to/repo                             # 打印 MCP/纪律/hook/Codex/pre-commit 接入片段,只打印不代写
...
iknowledge brief --repo /path/to/repo [--budget 1200]              # 防投毒数据框内的一屏 Markdown;头尾计入并严守 300..4000 预算
...
- 第一期单 repo 单实例、明文 HTTP、仅监听回环地址;缺省不鉴权。**威胁边界显式声明(定案)**:
  ...
  `hook` 桥自动携带,但同样先做无秘密挑战探活;`stdio` 桥在探活前读取 token,
  第一跳不带秘密:无鉴权服务返回 200 即拒绝;静态 token SHA-256 指纹只作诊断,
  凭证发送前必须通过随机 256-bit nonce + HMAC-SHA256(token,nonce) 持钥证明。
  缺证明的旧服务/普通 401 Bearer challenge/只重放静态指纹均拒绝(升级后须重启旧 serve,
  防同机占端口者伪造响应窃取 token);确认后 `/status` 与 MCP 转发
  才带同一 Bearer。自动拉起时 token 在位即追加 `serve --auth`,禁止
  "已鉴权服务探活 401 → 误判离线 → 另起无鉴权服务"的降级链。token 文件不存在才表示
  未启用鉴权;一旦存在就必须是 64 位 hex 且 Unix 权限不得开放给 group/other,否则读取
  fail closed;显式 `serve --auth` 可把既有权限收紧回 0600,不得把损坏解释成无鉴权。
  明文 HTTP 的网络窃听、主动同机端口代理转发不在缓解范围(监听非回环时警示文案区分
  有无 --auth);TLS 维持不做——回环 + token 覆盖直接未授权访问,更强同机威胁靠 OS 隔离,
...
- **许可证(已销案,2026-07-08)**:MIT 已定案并落 `LICENSE`,双语 README 同步声明;不再是发布阻塞项。
...
依赖方向:`cmd → {mcpserv,engine,store,buildinfo}`、`mcpserv → {engine,buildinfo}`、
`engine → {store,index,parser,model,pty}`;`buildinfo`/`model`/`pty` 不依赖其他内部包。
...
结构标签只遍历自由文本字段并原地脱敏;响应在成功时追加命中数与类型。模型中的 Entry/
...
附一个**可选** pre-commit 预检片段(安全 POSIX 单引号路径;缺省 warn-only,不自动安装/覆盖既有 hook):
