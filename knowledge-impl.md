...
iknowledge semantic <configure|enable|disable|status|rebuild|clear> # 可选语义检索 preview;默认禁用,配置与重建均须显式
...
可选语义 preview 已拆成两个标准库-only 包:`internal/semantic/` 承载薄 `Embedder` 与
OpenAI-compatible/Ollama HTTP provider;`internal/vector/` 承载 Flat backend、codec 与
不可变 snapshot。依赖只增加 `engine → {semantic,vector}`;两包都不反向依赖
engine/store/index/model,也不引入 Eino/eino-ext。当前已对外作为默认禁用的 preview;
质量晋级门未过,不能写成默认检索路径。
...
- **`.knowledge/local/`(新增)**:仓内不进 git 的非秘密本地态——`.lock`、使用日志
  ...
  可选语义 preview 另有可删重建、永不进 bundle 的 `vector.idx`(不含摘要正文/凭据,
...
- **仓外用户私有态**:auth token、local identity、scout trust、事务 WAL 按 canonical repo
  ...
  revision/enabled 与 top_k/min_score/max_vector_mib/timeout_seconds 也只写这里的
  `semantic-config-v1.json`;有效模式由 enabled+endpoint 推导,不另存 mode;仓内 config、bundle、MCP 参数无对应字段,
  API key 固定只读 `IKNOWLEDGE_EMBEDDING_API_KEY`。父链在信任根以下逐组件拒 symlink,
...
- **config fail-closed(轮 34)**:精确要求 schema=1、port 1..65535、include/exclude 均为
  ...
  **语义 provider 不是仓内 config 的扩展项**:当前实现对 `.knowledge/config.yaml`
  出现 endpoint/model/dimensions/revision/enabled 等 semantic 字段也不得开启任何网络行为;
  只有 §8.1 的仓外私有状态能授权。
...
当前 `tools/list` 共 **16 个工具**;分期列只保留来时路,均已交付。
...
行为: query 先按节点 ID 精确匹配(§3 文法,含宽松归一匹配),否则走关键词倒排(§8);
      ...
      provider;普通意图查询才让 lexical/trigram 与 semantic 各自产生有界候选,用固定 RRF
      融合;semantic record 合并前只对照当前 immutable source manifest 校验 source hash
      与当前节点存在性,候选输出展示当前 node status 及 keyword/semantic/RRF rank,
      再复用既有结构邻居扩一跳(调用/被调、接口/实现、同流程步骤)。精确节点下钻仍按既有
      usage/history 逻辑显示负知识与历史。
...
行为: 复用倒排索引反向匹配症状,优先带 pitfall;再聚合 Flow.Troubleshoot 与命中节点
      ...
      已知区域走 kb_recall、完全无头绪走 kb_investigate。可选语义层首版**不得**接入本工具:
      当前 Diagnose 搜 active pitfall/全部条目,而向量首版只收 summary/era_summary;只有形成
      可追溯 pitfall 摘要并单独通过评测后才能扩展,不能自动继承 Recall 的语义通道。
...
- **模式/授权**:默认 disabled;用户只能经 `iknowledge semantic configure/enable`
  在 canonical repo 对应的仓外私有 `semantic-config-v1.json` 选择 loopback Ollama 或 remote
  HTTPS OpenAI-compatible endpoint。它们是 `internal/semantic` 的 provider,不是第三方 MCP。
  `configure` 保存并 enable,但不联网、不自动重建;`enable` 只用于重新启用既有配置。
  API key 只读固定环境变量。仓内 YAML/journal/config、bundle、
  import 和 MCP 入参都不能创建/修改该状态,所以未获本机私有 enabled 时仓库内容不能外发。
  serve 启动/status 不探测 provider,批量重建只响应显式 `semantic rebuild`;stale recall 直接降级。
- **真实记录**:不存在 `Node.Summary`。record 只来自健康 index snapshot 中
  `Entry.Active() && KindSummary` 的稳定 record ID 与非空 `Node.EraSummary`;
  orphaned 节点整节点排除。先做现有 secret redaction,拼入有版本的 canonical node/kind
  前缀,再把最终 provider 文档截断为 4KiB 并计算 source hash/embedding。原始源码、其他 Entry
  全文、findings/WIP/session/usage 都不进向量。命中用 record ID 对照当前 immutable
  source manifest,只校验 node ID/source hash 与当前节点存在性;缓存不存正文。
- **immutable source manifest 与 generation**:健康 tree/project generation 变化时递增
  source version;按 version 缓存 immutable records map/fingerprint,稳态 recall O(1) 读它而不再全库扫描。
  变代后仅在 `rt.mu` 内复制 immutable string headers;脱敏、截断、排序与哈希在主锁外
  重建,并仅在 source version 未变时整体发布。每仓 `rebuildMu` 串行显式重建;
  HTTP、Flat build、codec 与磁盘写入不持主锁,发布前重查 source/settings fingerprint。
  持久 metadata **只**存 schema、generation、settings/embedder/probe/source fingerprints、dimensions、records、built_at,
  不单列 raw model/revision;内存完整 immutable snapshot 一次替换。
- **context**:`mcpserv` 将请求 context 传给 `RecallContext`,并贯穿 Embedder、
  `Snapshot.Search` 与 vector codec;`semantic rebuild` CLI 用 signal context 响应 SIGINT/SIGTERM。
  HTTP timeout 与调用方/CLI 取消能中断这些已接通的路径;不将它扩大为所有
  server shutdown 或任意阻塞 I/O 都可立即取消。
- **融合**:确定性 exact 查询先短路;其余 lexical/trigram 与 semantic 候选分池、
  有界、去重后按 RRF 融合。semantic hit 只校验当前 manifest source hash 和节点存在性;
  输出当前 node status 及 keyword rank/score、semantic rank/cosine、RRF,再复用既有结构邻居
  扩一跳(调用/被调、接口/实现、同流程步骤)。
  当前不做逐条 confidence/dispute/lineage/supersedes 裁决、状态降权、负知识优先排序或
  独立 flow 向量通道/candidate pool;“同流程步骤”仅是 RRF 后的既有结构扩展。精确 node
  下钻保留既有负知识/history 展示。首版只接 `kb_recall`,
  不接 `kb_diagnose`。
- **缓存/status/资源**:`.knowledge/local/vector.idx` 是 0600、带版本头/checksum
  ...
  revision。冷进程 `semantic status` 只校验固定 wrapper 与 `≤64KiB` metadata/binding;
  payload 的有界 decode 和完整 checksum 校验延迟到首次 semantic recall。provider 通用单条
  文本硬限 16KiB,rebuild 文档为 4KiB、batch=32,request/response 上限为 1MiB/16MiB,
  每仓 provider 在途请求上限为 1;拿到 slot 后 query/probe 会二次查缓存。dimensions
  为 0(auto)或 1..4096,实际 snapshot 为 1..4096;每仓 100k record、vector 512MiB、top_k 100
  均为硬上限,单次 HTTP timeout 硬上限 30s。工作集有界且超限时 semantic 通道报
  resource limit 并回退 lexical/structural;这不是“绝不会 OOM”的绝对承诺。
...
- **质量门**:至少 100 条固定中英 query/qrels;精确控制组 100% 不退化,同义改写 Recall@10
...
`configure/enable/disable/status/clear` 不调用 provider;`rebuild` 是唯一显式批量发送摘要的
命令,`clear` 只删派生索引并保留 provider 配置。
...
16 个 kb_* 工具描述里的微纪律兜底——工具描述是"连接存在即必在上下文"的钩子(serena 同款);
...
且校验通过才安装;否则拒绝该二进制并回退 `go install`。`IKNOWLEDGE_FORCE_SOURCE`
在任何 release 请求前生效,`IKNOWLEDGE_BIN` 同时决定 binary/source 的落点;Windows
全程使用 `.exe`。curl|sh 时绝不把 cwd 冒充脚本目录读取仓库内 skill,只有真实本地
`install.sh` 才可复用邻接资源。脚本由用户显式执行、只写用户级 binary/skill 目录,
在铁律二边界之外。两侧均实测通过
...
`uninstall.sh`(停全部 serve、对称删除 `~/.local/bin`/`IKNOWLEDGE_BIN`/Go bin 与 `.exe`、
...
- `semantic/vector`(可选语义 preview):fake Embedder 确定性;真实 active summary/EraSummary
  manifest;provider/source fingerprint 与重建中变代丢弃;immutable snapshot 并发 Search/replace
  race;截断、坏 checksum、NaN/Inf/零向量、维度/计数/乘法溢出/超限文件 fail closed;
  ...
  降级与显式 rebuild 报错;context cancel 分别覆盖 Recall/Embedder/Search/codec 与 CLI
  signal 的取消传播;固定 qrels 对 lexical-only 与 hybrid 的 Recall/nDCG/MRR 门。
